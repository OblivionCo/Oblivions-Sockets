...
namespace OblivionsSockets.NPCs
{
	// [AutoloadHead] and NPC.townNPC are extremely important and absolutely both necessary for any Town NPC to work at all.
	[AutoloadHead]
	public class FragmentTinkerer : ModNPC
	{
		...
		*/
		...
		public override bool CanTownNPCSpawn(int numTownNPCs, int money) { // Requirements for the town NPC to spawn.
			for (int k = 0; k < 255; k++) {
				...
				if (player.inventory.Any(item => item.type == ModContent.ItemType<OblivionsEdge>()/* || item.type == ModContent.ItemType<Items.Placeable.ExampleBlock>()*/)) {
					return true;
				}
			}
		...
		public override ITownNPCProfile TownNPCProfile() {
			return new FragmentTinkererProfile();
		}
		...
		public override void ModifyNPCLoot(NPCLoot npcLoot) {
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<OblivionsEdge>()));
		}
	...
	public class FragmentTinkererProfile : ITownNPCProfile
	{
		...
		public Asset<Texture2D> GetTextureNPCShouldUse(NPC npc) {
			if (npc.IsABestiaryIconDummy && !npc.ForcePartyHatOn)
				return ModContent.Request<Texture2D>("OblivionsSockets/NPC/FragmentTinkerer");

			if (npc.altTexture == 1)
				return ModContent.Request<Texture2D>("OblivionsSockets/NPC/FragmentTinkerer_Party");

			return ModContent.Request<Texture2D>("OblivionsSockets/NPC/FragmentTinkerer");
		}

		public int GetHeadTextureIndex(NPC npc) => ModContent.GetModHeadSlot("OblivionsSockets/NPC");
	}
}